...
traindataaux = np.array(traindata[:,:,24:-1]) #22
traindata = np.array(traindata[:,:,:22])
valdataaux = np.array(valdata[:,:,24:-1]) # 22
valdata = np.array(valdata[:,:,:22])
...
adam = Adam(lr=0)
model.summary()
best_model_file = './bestaccseqpssmmultitask83e-4.h5' 
...
score = model.evaluate({'main_input': valdataint, 'aux_input': valdataaux}, {'main_output': vallabel, 'aux_output': valsolvlabel})
print(score)
pred = model.predict({'main_input': valdataint, 'aux_input': valdataaux})
print(len(pred), pred[0].shape, pred[1].shape)
print(pred[0].shape, pred[0][0,0,:])
...
for i in xrange(vallabel.shape[0]):
    for j in xrange(vallabel.shape[1]):
        if vallabel[i,j,:].sum() == 1:
            ...
            labelpd[nsample,np.argmax(pred[0][i,j,:])] = 1
            if vallabel[i,j, np.argmax(pred[0][i,j,:])] == 1:
                nacc += 1
            nsample += 1
        else:
